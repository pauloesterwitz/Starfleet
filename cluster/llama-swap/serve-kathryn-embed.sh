#!/bin/bash
# serve-kathryn-embed.sh — run an embedding model (nomic-embed-text | embeddinggemma) on
...
#
#   cmd: serve-kathryn-embed.sh <nomic-embed-text|embeddinggemma> --port ${PORT}
#   proxy: http://127.0.0.1:${PORT}
set -uo pipefail

KEY="${1:?usage: serve-kathryn-embed.sh <nomic-embed-text|embeddinggemma> --port <port>}"
shift
PORT=8000
